...
<img src="https://latex.codecogs.com/svg.latex?\D^*_G(x) = {q_{data}(x) \over q_{data}+p_G(x)} = sigmoid(f^*(x)), \\where\ f^*(x) = \log q_{data}(x) -\log p_G(x)" title = "\D^*_G(x) = {q_{data}(x) \over q_{data}+p_G(x)} = sigmoid(f^*(x)), \\where\ f^*(x) = \log q_{data}(x) -\log p_G(x)" />
