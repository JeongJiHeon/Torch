...
G = Generator()
D = Discriminator()
G = utils.load_model(G, name = 'cgan_W_Generator.pkl')
D = utils.load_model(D, name = 'cgan_W_Discriminator.pkl')
mnist = utils.dataset('mnist')
#criterion = nn.BCELoss()
optimG = torch.optim.Adam(G.parameters(),lr = 0.0005, betas = (0.5, 0.9))
optimD = torch.optim.Adam(D.parameters(),lr = 0.0005, betas = (0.5, 0.9))
...
#            D_loss = criterion(D(real, y_real), real_label) + criterion(D(fake, y_fake),fake_label)
            D_real = D(real, y_real).mean()
            D_real.backward(one, retain_graph=True)

            D_fake = D(fake, y_fake).mean()
            ...
            if i % 50 == 0:
                ...
                utils.save_model(G, name = 'cgan_W_Generator.pkl')
                utils.save_model(D, name = 'cgan_W_Discriminator.pkl')
            if i % 50 == 0 :
                if 'cgan_W_imglist.npy' in os.listdir():
                    img_list = list(np.load('cgan_W_imglist.npy'))
                img_list.append(G(fix_latent, fix_y).detach().numpy())
                np.save('cgan_W_imglist', img_list)
